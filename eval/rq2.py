...
for LANG in ["java", "py", "cpp"]:
    ...
    for SAMPLE_NUM in range(1,6):
        ...
        print("--------------------------------------")
        print("Scores for question:", LANG, str(SAMPLE_NUM))
        '''
        ...
        for k, average in averages:
            if k.startswith("Claude"):
                claude_avg = average
                data[f"{LANG}{SAMPLE_NUM}"]["Claude No Tools"] = round(average, 2)
                print(f"{k:<{max_key_length}}: {average:.2%}")
            else:
                diff = average - claude_avg
                data[f"{LANG}{SAMPLE_NUM}"][f"Agentic Claude ({k})"] = round(average, 2)
